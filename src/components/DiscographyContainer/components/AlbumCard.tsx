...
export default function AlbumCard({ album }: { album: any }) {
  ...
  return (
    <div key={album.id} className='bg-white shadow-xl hover:scale-105 transition-all duration-300 px-4 py-8'>
      <div className='flex flex-col gap-4'>
        ...
        <div className="relative self-center">
          <img
            src={album.coverArt}
            alt={album.title}
            className="w-36 h-36 rounded-full object-cover border-4 border-white shadow-xl ring-2 ring-black"
          />
          {/* Filtro solo sobre la imagen */}
          <div className="absolute inset-0 bg-white/30 backdrop-saturate-300 rounded-full z-10"></div>
        </div>
        <span className={`font-playfair-display-sc text-xl font-bold text-gray-800 text-center`}>{album.title}</span>

        <PlatformsSection album={album} />
      </div>
    </div>
  )
};
...
const PlatformsSection = ({ album }: { album: any }) => {
  return (
